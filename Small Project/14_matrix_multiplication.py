...
if matrix_A_cols == matrix_B_rows:
    ...
    matrix_A = [[int(input(f"column {j+1} Row {i+1} element:")) for j in range(matrix_A_cols)] for i in range(matrix_A_rows) ]

    print() # for new line

    print('Enter values for matrix B ')
    matrix_B = [[int(input(f"column {j+1} Row {i+1} element:")) for j in range(matrix_B_cols)] for i in range(matrix_B_rows) ]
